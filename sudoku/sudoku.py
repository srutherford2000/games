...
def check_square(num):
    global mistake, error, error_msg
    if sudoku[int(chosen_block)] == " ":
        if num ==solution[int(chosen_block)]:
            sudoku[int(chosen_block)] = num
        else:
            mistake += 1
            error_msg = "that number was wrong!"
            error = True
    else:
        error_msg ="this was a given number! don't try to change it"
        error = True
...
def check_numbers():
    ...
    if keyboard.k_1 and start:
            check_square("1")
            start = False
            dict[chosen_block].image = 'square'
...
def on_mouse_down(pos):
    ...
    for box in dict:
        if dict[box].collidepoint(pos):
            ...
            start = True
def draw():
    global sudoku,start, chosen_block, mistake, clock,error,error_msg
    screen.fill("white")
    for box in dict:
            dict[box].draw()
    if start:
        dict[chosen_block].image = 'gray'
    vert1.draw()
    vert2.draw()
    hor1.draw()
    hor2.draw()
    ...
    if error:
        print("here")
        screen.draw.text(error_msg,(20,563),color="red", fontsize=30)
        time.sleep(2)
        error = False
    else:
        screen.draw.text("You have "+str(mistake)+" mistakes",(20,563),color="black", fontsize=30)
        screen.draw.text("Time: "+str(int(time.monotonic()-clock)),(450,563),color="black", fontsize=30)
